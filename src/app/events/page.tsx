...
// --- TYPES ---
// Defined locally since we are fetching from API
    ...
    <div className="absolute inset-0 border border-white/10 rounded-xl bg-zinc-900/50 overflow-hidden transition-all duration-300 group-hover:border-oz-emerald/50 group-hover:shadow-[0_0_30px_rgba(16,185,129,0.15)]">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 z-20 bg-gradient-to-t from-black via-black/90 to-transparent pt-12">
          ...
          <div className="flex items-center justify-between text-xs font-mono text-gray-500 border-t border-white/10 pt-4">
            <span className="flex items-center gap-1 text-oz-emerald/80"><Calendar size={12}/> {new Date(event.date).toLocaleDateString()}</span>
...
const EventModal = ({ event, onClose }: { event: Event; onClose: () => void }) => {
  ...
  // 3. (Status is Live OR (It is Upcoming AND Current Time > Reg Open Time))
        ...
        >
          ...
          <div className="w-full md:w-7/12 p-6 md:p-10 flex flex-col bg-zinc-950/95 backdrop-blur-3xl overflow-y-auto custom-scrollbar">
              <div className="mb-6 flex items-center justify-between pr-8">
                <StatusBadge status={event.status} />
                <span className="text-xs font-mono text-gray-500 border border-white/10 px-2 py-1 rounded hidden sm:inline-block">ID: {event.id}</span>
              </div>
              ...
              <div className="flex flex-wrap gap-4 text-sm text-gray-400 font-mono mb-8 py-4 border-y border-white/5">
                <span className="flex items-center gap-2"><Calendar size={16} className="text-oz-emerald"/> {new Date(event.date).toLocaleDateString()}</span>
                <span className="flex items-center gap-2"><MapPin size={16} className="text-oz-emerald"/> {event.location}</span>
              </div>
              <div className="flex-grow">
                <h4 className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">Mission Brief</h4>
                <p className="text-gray-300 leading-relaxed mb-8 text-sm md:text-base whitespace-pre-line">{event.fullDescription}</p>
              </div>
              <div className="mt-8 pt-6 border-t border-white/5 shrink-0">
                {isRegistrationActive ? (
                  <a href={event.registrationLink || "#"} target="_blank" rel="noreferrer" className="w-full py-4 bg-oz-emerald text-amber-50 hover:text-black font-bold font-orbitron uppercase tracking-wider hover:bg-white transition-all rounded-sm flex items-center justify-center gap-2 group shadow-[0_0_20px_rgba(16,185,129,0.3)] hover:shadow-[0_0_30px_rgba(16,185,129,0.5)] cursor-pointer">
                    ...
                    {event.status === 'upcoming' ? 'Initialize Registration' : 'Initialize Registration'}
                  </a>
...
export default function EventsPage() {
  ...
  const activeEvents = filteredBase.filter(e => ['live', 'upcoming'].includes(e.status));
      ...
      <div className="pt-32 pb-20 px-6 max-w-7xl mx-auto">
        ...
        <div className="flex flex-wrap gap-6 mb-16">
           ...
           <div className="flex gap-2 overflow-x-auto no-scrollbar">
              <button onClick={() => setSelectedCategory("All")} className={`px-4 py-2 rounded-full text-xs font-bold border transition-all whitespace-nowrap ${selectedCategory === "All" ? "bg-oz-emerald text-black border-oz-emerald" : "border-white/10 text-gray-400 hover:border-white"}`}>ALL</button>
              {categories.map((cat) => (
        ...
        {/* Loading State */}
        {loading && <div className="text-center py-20 text-gray-500 font-mono animate-pulse">Initializing Data Stream...</div>}
        ...
        {!loading && activeEvents.length > 0 && (
          <div className="mb-24">
            <div className="flex items-center gap-3 mb-8">
              ...
              <h2 className="text-2xl font-bold font-orbitron tracking-wide text-white">ACTIVE SIGNALS</h2>
