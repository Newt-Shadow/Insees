...
export const MemberCard = ({ name, por, img, socials }: MemberProps) => {
  ...
  return (
    ...
    >
      {/* Image Container */}
      <div className="w-full h-48 bg-gray-700 relative group overflow-hidden">
        {hasImg ? (
          <Image
            src={img}
            alt={name}
            fill // ✅ "fill" makes it adapt to the parent div (h-48)
            sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" // ✅ Tells browser to download small version
        ...
        {/* Overlay Effect */}
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition duration-500 z-10" />
      </div>

      {/* Content */}
      <div className="p-5 relative z-20"> {/* Added z-20 to ensure text sits above overlay */}
        ...
        <div className="flex items-center justify-center gap-6 mt-5 text-gray-300 text-lg">
          {socials.instagram && (
            <a href={socials.instagram} target="_blank" rel="noreferrer" className="hover:text-pink-500 transition-colors">
              <FaInstagram />
            </a>
          )}
          {socials.facebook && (
            <a href={socials.facebook} target="_blank" rel="noreferrer" className="hover:text-blue-500 transition-colors">
              <FaFacebook />
            </a>
          )}
          {socials.linkedin && (
            <a href={socials.linkedin} target="_blank" rel="noreferrer" className="hover:text-sky-500 transition-colors">
              <FaLinkedin />
            </a>
          )}
          {socials.github && (
            <a href={socials.github} target="_blank" rel="noreferrer" className="hover:text-gray-400 transition-colors">
              <FaGithub />
            </a>
          )}
        </div>
